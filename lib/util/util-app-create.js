 ...
 * @requires lib/templates/template-component-index-react.js:TEMPLATE_COMPONENT_INDEX_REACT
 * @requires lib/templates/template-component-style-block.js:TEMPLATE_COMPONENT_STYLE_BLOCK
 */
const fs = require('fs');
...
const appPathBuild = (appName, fileType) => {
  ...
  if(appName) {
    ...
    const DIR_ROOT = process.cwd(),
          ...
          fileContent = fileType === "js" ? TEMPLATE_COMPONENT_INDEX_REACT(appName) : TEMPLATE_COMPONENT_STYLE_BLOCK(appName);

    appStructureCreate(appDirPath, appFileIndexPath, fileContent);

  } else {

    console.log('No app name inputed, please run: npm run app <app-name>');
    process.exit();
