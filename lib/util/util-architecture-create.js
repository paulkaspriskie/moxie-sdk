 ...
 */
const archPathBuild = async (inputValue, fileType) => {
  ...
  if(inputValue) {
    ...
    if(cmdType === 'create-app') {
      ...
      const DIR_ROOT = process.cwd(),
            appDirPath = path.resolve(DIR_ROOT, `apps/${inputValue}/src/${fileType}`),
            appFileIndexPath = path.resolve(DIR_ROOT, `apps/${inputValue}/src/${fileType}/index.${fileType}`),
            fileContent = fileType === "js" ? TEMPLATE_COMPONENT_REACT_INDEX(inputValue) : TEMPLATE_COMPONENT_STYLE_BLOCK(inputValue);

    archCreate(appDirPath, appFileIndexPath, fileContent);

    } else if (cmdType === 'create-component') {

      let test = await promptResponse('test: ');
      console.log(test);
...
(async () => {
  await archPathBuild(cliInput, "js");
  await archPathBuild(cliInput, "scss");
})();
