 ...
 * @requires lib/templates/template-architecture-javascript.js:TEMPLATE_COMPONENT_REACT_INDEX
 * @requires lib/templates/template-architecture-javascript.js:TEMPLATE_COMPONENT_REACT
 * @requires lib/templates/template-architecture-styles.js:TEMPLATE_STYLE_APP_BLOCK
 * @requires lib/templates/template-architecture-styles.js:TEMPLATE_STYLE_COMPONENT_BLOCK
...
const inputArr = require('node:process').argv.slice(2);
...
const { TEMPLATE_STYLE_APP_BLOCK, TEMPLATE_STYLE_COMPONENT_BLOCK } = require('../templates/templates-architecture-styles.js');
const { dirFilter, fileCreate, kebabCase, pascalCase, promptResponse } = require('./util-functions.js');
  ...
  if(inputValue) {
    ...
    data.pathData.map(async (item, i) => {
      ...
      if(cmdType === 'create-app') {
        ...
        let jsDirPath = archBuildPath(cmdType, kebabCase(inputArr), item.jsDirPath),
            jsFilePath = archBuildPath(cmdType, kebabCase(inputArr), item.jsFilePath),
            scssDirPath = archBuildPath(cmdType, kebabCase(inputArr), item.scssDirPath),
            scssFilePath = archBuildPath(cmdType, kebabCase(inputArr), item.scssFilePath);

        archBuildSystem(jsDirPath, jsFilePath, TEMPLATE_COMPONENT_REACT_INDEX(inputArr));
        archBuildSystem(scssDirPath, scssFilePath, TEMPLATE_STYLE_APP_BLOCK(inputArr));
        ...
        let jsComponentDir = archBuildPath(cmdType, appName, item.jsComponentDir),
            jsComponentFile = archBuildPath(cmdType, appName, item.jsComponentDir, `${pascalCase(inputArr)}.js`),
            scssComponentDir = archBuildPath(cmdType, appName, item.scssComponentDir),
            scssComponentFile = archBuildPath(cmdType, appName, item.scssComponentDir, `_${kebabCase(inputArr)}.scss`);

        if(appsArray.includes(appName)) {

          archBuildSystem(jsComponentDir, jsComponentFile, TEMPLATE_COMPONENT_REACT(inputArr));
          archBuildSystem(scssComponentDir, scssComponentFile, TEMPLATE_STYLE_COMPONENT_BLOCK(inputArr));
